...
import { HiCalculator, HiMapPin, HiBolt, HiArrowRight, HiShieldCheck, HiClock } from 'react-icons/hi2';
...
export default function CotizarExpressPage() {
 ...
 // Estimación de tarifa base: base + por km
 const tarifaCalculada = Math.round(2500 + distanciaKm * 500);

 const handleCalcular = (e: React.FormEvent) => {
 e.preventDefault();
 if (!origen || !destino) return;
 // Simular variación según longitud de texto o distancia
 const simulatedDist = Math.max(1.5, Math.min(18, Number((Math.random() * 8 + 2).toFixed(1))));
 setDistanciaKm(simulatedDist);
 setIsCalculated(true);
 };

 const whatsappLink = https://wa.me/542236602699?text=Hola,%20quiero%20pedir%20un%20Env%C3%ADo%20Express:%0A- Origen:%20%0A- Destino:%20%0A- Tarifa Estimada:%20{tarifaCalculada.toLocaleString('es-AR')};
 ...
 <HiClock className=\text-brand-yellow text-base shrink-0\ />
 <span>&lt; 90 min</span>
 </div>
 <div className=\bg-brand-white/10 border border-brand-white/20 p-2.5 rounded-xl flex items-center gap-2\>
 <HiMapPin className=\text-brand-yellow text-base shrink-0\ />
 <span>Ruta Directa</span>
 </div>
 <div className=\bg-brand-white/10 border border-brand-white/20 p-2.5 rounded-xl flex items-center gap-2\>
 <HiShieldCheck className=\text-brand-yellow text-base shrink-0\ />
 <span>Seguro</span>
 </div>
 </div>
 </div>
 ...
 <HiCalculator className=\text-3xl text-brand-blue\ />
 </div>
 ...
 <HiMapPin className=\text-sm text-brand-blue\ /> Origen (Retiro)
 </label>
 <input
 type=\text\
 value={origen}
 onChange={(e) => setOrigen(e.target.value)}
 placeholder=\Ej: Av. Colón 1234 MDQ\
 ...
 <div className=\space-y-1\>
 <label className=\font-subheading uppercase text-xs font-bold text-brand-blue flex items-center gap-1\>
 <HiMapPin className=\text-sm text-brand-blue\ /> Destino (Entrega)
 </label>
 <input
 type=\text\
 value={destino}
 onChange={(e) => setDestino(e.target.value)}
 placeholder=\Ej: Güemes 2840 MDQ\
 className=\w-full h-11 px-3.5 bg-white border-2 border-brand-blue/20 rounded-xl text-brand-blue font-sans text-xs focus:outline-none focus:border-brand-blue\
 />
 </div>
 ...
 <button
 type=\submit\
 className=\w-full py-3 rounded-xl bg-brand-blue text-brand-yellow font-subheading uppercase tracking-wider font-bold text-xs hover:bg-brand-blue-deep transition-colors shadow-md\
 >
 Calcular Tarifa
 </button>
 </form>
 ...
 <span>Confirmar Envío por WhatsApp</span>
 </a>
 </div>
 )}
 </div>
